...
- сцена может быть тактической картой или крупной иллюстрацией без лишних панелей поверх изображения.
...
- `presentation_mode=tactical_map` показывает токены, fog of war и GM-инструменты;
- `presentation_mode=illustration` показывает изображение крупно и скрывает тактические слои;
...
- выбрать режим сцены и отображение изображения;
...
- видеть портреты персонажей и открытых NPC, если они заданы;
...
Сцены и режимы отображения описаны в `docs/scenes.md`.
Слой токенов описан в `docs/tokens.md`.
Fog of War описан в `docs/fog-of-war.md`.

## Портреты

Персонажи используют `avatar_attachment_id`, NPC используют `portrait_attachment_id`.

Frontend показывает круглую превью-картинку в карточках персонажей и NPC. Если портрета нет, показывается круглая заглушка с первой буквой имени.

Токен на карте выбирает изображение в таком порядке:

1. `scene_token.image_attachment_id`;
2. portrait персонажа;
3. portrait NPC;
4. буква имени или иконка маркера.
...
- полноценного combat, marketplace, оплаты и WebSocket-чата с постоянной историей соединений пока нет;
- VTT-карта, сцены, токены, fog of war и базовый realtime уже реализованы как фундамент Play Room;
